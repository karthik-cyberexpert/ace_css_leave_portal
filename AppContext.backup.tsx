...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const syncStudentStatusWithBatch = async (batchId: string, isActive: boolean) => {
    try {
      ...
      if (response.data.updatedCount > 0) {
        console.log(`âœ… Updated ${response.data.updatedCount} students to ${isActive ? 'active' : 'inactive'} status for batch ${batchId}`);
        ...
        console.log(`â„¹ï¸ No students found to update for batch ${batchId}`);
        return { success: true, updatedCount: 0 };
      }
    } catch (error: any) {
      console.warn(`âš ï¸ Failed to sync student status for batch ${batchId}:`, error.response?.data?.error || error.message);
  ...
  }, [handleLogout]); // Only depend on handleLogout, not sessionManager
  ...
  }, [session, resetInactivityTimer, updateActivity]); // Removed sessionManager.timeoutId to prevent infinite loop
