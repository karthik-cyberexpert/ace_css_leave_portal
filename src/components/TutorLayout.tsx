...
const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => {
  ...
  return (
    <>
      <Link to="/tutor-profile" className="flex items-center space-x-3 p-4 mb-6 hover:bg-sidebar-accent rounded-xl transition-all duration-300 hover:shadow-beautiful group">
        <Avatar className="h-10 w-10 ring-2 ring-sidebar-border group-hover:ring-primary/50 transition-all">
          ...
          <AvatarFallback className="bg-sidebar-accent text-sidebar-accent-foreground"><UserCircle className="h-10 w-10" /></AvatarFallback>
        </Avatar>
        <span className="font-semibold text-lg text-sidebar-foreground group-hover:text-primary transition-colors">{currentTutor?.name || 'Tutor'}</span>
      </Link>
