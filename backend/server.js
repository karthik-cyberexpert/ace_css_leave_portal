...
// Clear all leave requests (Admin only)
app.delete('/admin/clear-leave-requests', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    const [user] = await query(
      'SELECT is_admin FROM users WHERE id = ?',
    ...
    const result = await query('DELETE FROM leave_requests');
    ...
    console.error(error);
    res.status(500).json({ error: 'Failed to clear leave requests' });
  }
});

// Clear all OD requests (Admin only)
app.delete('/admin/clear-od-requests', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    const [user] = await query(
      'SELECT is_admin FROM users WHERE id = ?',
    ...
    const result = await query('DELETE FROM od_requests');
    
    res.json({ 
      message: 'All OD requests cleared successfully',
      deletedCount: result.affectedRows
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Failed to clear OD requests' });
  }
});

// Clear all requests and reset student leave counts (Admin only)
app.delete('/admin/clear-all-requests', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    const [user] = await query(
      'SELECT is_admin FROM users WHERE id = ?',
    ...
    // Delete all leave requests
    const leaveResult = await query('DELETE FROM leave_requests');
    
    // Delete all OD requests
    const odResult = await query('DELETE FROM od_requests');
    
    // Reset all student leave counts to 0
    const resetResult = await query('UPDATE students SET leave_taken = 0');
    
    res.json({ 
      message: 'All requests and student leave history cleared successfully',
      clearedLeaveRequests: leaveResult.affectedRows,
      clearedODRequests: odResult.affectedRows,
      resetStudentLeaveCount: resetResult.affectedRows
    });
  } catch (error) {
    console.error(error);
